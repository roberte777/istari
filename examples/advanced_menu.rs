...
fn main() -> io::Result<()> {
    ...
    counter_menu.add_action(
        'i',
        "Increment (optional amount)",
        |state: &mut AdvancedAppState, params: Option<&str>| {
    ...
    counter_menu.add_action(
        ...
        |state: &mut AdvancedAppState, params: Option<&str>| {
            ...
            state
                .history
                .push(format!("Reset counter from {} to {}", old_value, new_value));
    ...
    settings_menu.add_action(
        ...
        |state: &mut AdvancedAppState, _params: Option<&str>| {
    ...
    user_menu.add_action(
        ...
        |state: &mut AdvancedAppState, params: Option<&str>| {
            if state.history.is_empty() {
                ...
                let history = state.history[start..]
                    .iter()
                    .enumerate()
                ...
                Some(format!(
                    "Action History (last {} items):\n{}",
                    state.history.len().min(limit),
                    history
                ))
            }
        },

...
fn main() -> io::Result<()> {
    ...
    counter_menu.add_action('i', "Increment (optional amount)", |state: &mut AdvancedAppState, params: Option<&str>| {
    ...
    counter_menu.add_action('d', "Decrement (optional amount)", |state: &mut AdvancedAppState, params: Option<&str>| {
        let amount = params.and_then(|p| p.parse::<i32>().ok()).unwrap_or(1);
        state.counter -= amount;
    ...
    counter_menu.add_action('r', "Reset (optional value)", |state: &mut AdvancedAppState, params: Option<&str>| {
    ...
    counter_menu.add_action('m', "Multiply (optional factor)", |state: &mut AdvancedAppState, params: Option<&str>| {
    ...
    settings_menu.add_action('t', "Change Theme", |state: &mut AdvancedAppState, params: Option<&str>| {
    ...
    settings_menu.add_action('n', "Toggle Notifications", |state: &mut AdvancedAppState, _params: Option<&str>| {
        state.settings.notifications = !state.settings.notifications;
        Some(format!("Notifications are now {}", 
            if state.settings.notifications { "enabled" } else { "disabled" }))
    });
    
    settings_menu.add_action('a', "Toggle Auto-save", |state: &mut AdvancedAppState, _params: Option<&str>| {
    ...
    user_menu.add_action('r', "Rename User", |state: &mut AdvancedAppState, params: Option<&str>| {
    ...
    // History viewer
    user_menu.add_action('h', "View History", |state: &mut AdvancedAppState, params: Option<&str>| {
    ...
    user_menu.add_action('c', "Clear History", |state: &mut AdvancedAppState, _params: Option<&str>| {
    ...
    root_menu.add_action('s', "Show Status", |state: &mut AdvancedAppState, _params: Option<&str>| {
        Some(format!(
            "Current Status:\n\
             - User: {}\n\
             - Counter: {}\n\

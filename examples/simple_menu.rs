...
fn main() -> io::Result<()> {
    ...
    root_menu.add_action('1', "Increment Counter (optional amount)", Box::new(|state: &mut AppState, params: Option<&str>| {
        let amount = params.and_then(|p| p.parse::<i32>().ok()).unwrap_or(1);
        state.counter += amount;
        Some(format!("Counter incremented by {} to {}", amount, state.counter))
    }));

    root_menu.add_action('2', "Decrement Counter (optional amount)", Box::new(|state: &mut AppState, params: Option<&str>| {
        let amount = params.and_then(|p| p.parse::<i32>().ok()).unwrap_or(1);
        state.counter -= amount;
    ...
    submenu.add_action('r', "Reset Counter (optionally to value)", Box::new(|state: &mut AppState, params: Option<&str>| {
        let value = params.and_then(|p| p.parse::<i32>().ok()).unwrap_or(0);
        state.counter = value;
        Some(format!("Counter reset to {}", value))
    }));

    submenu.add_action('d', "Double Counter (optional multiplier)", Box::new(|state: &mut AppState, params: Option<&str>| {
        let multiplier = params.and_then(|p| p.parse::<i32>().ok()).unwrap_or(2);
        state.counter *= multiplier;
        Some(format!("Counter multiplied by {} to {}", multiplier, state.counter))
    }));

    // Add a silent action that doesn't produce output
    submenu.add_action('s', "Silent Update (optional amount)", Box::new(|state: &mut AppState, params: Option<&str>| {
    ...
    submenu.add_action('l', "Generate Log Output (optional lines)", Box::new(|state: &mut AppState, params: Option<&str>| {
        let lines = params.and_then(|p| p.parse::<usize>().ok()).unwrap_or(50);
        let mut output = String::new();
        for i in 1..=lines {
            output.push_str(&format!("Log line {}: Counter is currently {}\n", i, state.counter));
        }
        Some(output)
    }));

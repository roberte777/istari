...
fn main() -> io::Result<()> {
    ...
    root_menu.add_action('1', "Increment Counter (optional amount)", |state: &mut AppState, params: Option<&str>| {
        let amount = params.and_then(|p| p.parse::<i32>().ok()).unwrap_or(1);
        state.counter += amount;
        Some(format!("Counter incremented by {} to {}", amount, state.counter))
    });

    root_menu.add_action('2', "Decrement Counter (optional amount)", |state: &mut AppState, params: Option<&str>| {
        let amount = params.and_then(|p| p.parse::<i32>().ok()).unwrap_or(1);
        state.counter -= amount;
    ...
    submenu.add_action('r', "Reset Counter (optionally to value)", |state: &mut AppState, params: Option<&str>| {
        let value = params.and_then(|p| p.parse::<i32>().ok()).unwrap_or(0);
        state.counter = value;
        Some(format!("Counter reset to {}", value))
    });

    submenu.add_action('d', "Double Counter (optional multiplier)", |state: &mut AppState, params: Option<&str>| {
    ...
    submenu.add_action('s', "Silent Update (optional amount)", |state: &mut AppState, params: Option<&str>| {
    ...
    submenu.add_action('l', "Generate Log Output (optional lines)", |state: &mut AppState, params: Option<&str>| {

                            ...
                            Mode::Scroll => {
                                ...
                                match c {
                                    ...
                                    'j' => {
                                        ...
                                        let output_messages = app.output_messages();
                                        let output_area_height = renderer.terminal.size()?.height as usize - 2;
                                        let content_height = output_messages.len();
                                        let max_scroll = content_height.saturating_sub(output_area_height);
                                    ...
                                    'u' => {
                                        // Half-page up
                                        let page_size = (renderer.terminal.size()?.height as usize - 2) / 2;
                                        renderer.output_scroll = renderer.output_scroll.saturating_sub(page_size);
                                        // Disable auto-scroll when manually scrolling up
                                        renderer.auto_scroll = false;
                                    },
                                    'd' => {
                                    ...
                                    'a' if key.modifiers.contains(crossterm::event::KeyModifiers::CONTROL) => {
